...
char *xe(char *params)
{
	char amount[PATH_MAX];
	char from[PATH_MAX];
	char to[PATH_MAX];
	char url[PATH_MAX];

	if(sscanf(params, "%s %s in %s", amount, from, to) != 3)
		return(NULL);
	snprintf(url, PATH_MAX, "http://frugalware.org/~vmiklos/xe/%s/%s/in/%s", amount, from, to);
	CURL *easyhandle;
...
{
	return(match(str, "^[0-9.]+[KM]? [a-z]+ in [a-z]+$"));
}
