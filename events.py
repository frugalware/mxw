...
def lc(c, source, target, data):
	...
	sock = urllib.urlopen('http://dawn.royalcomp.hu/~raas/lc.html?format=json')
	num = json.loads(sock.read())['counter']
	sock.close()

	c.privmsg(target, "Congratulations, you successfully incremented the LAMMERCOUNTER! %s" % num)
